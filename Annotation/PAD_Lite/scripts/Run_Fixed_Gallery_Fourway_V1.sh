...
run_root="${pad_root}/outputs/04_gallery_and_recognizer_evaluation/fixed_gallery_fourway_v1_00/${run_id}"
...
mkdir -p "${run_root}"
cd "${annotation_root}"
"${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway validate \
  --config "${config}" --run-root "${run_root}"

(
  CUDA_VISIBLE_DEVICES="${dino_gpu}" "${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway raw \
    --config "${config}" --run-root "${run_root}" --backbone dino_raw --device cuda:0
  CUDA_VISIBLE_DEVICES="${dino_gpu}" "${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway dino-p2b \
    --config "${config}" --run-root "${run_root}" --device cuda:0
) >"${run_root}/dino_chain.log" 2>&1 &
dino_pid=$!

(
  CUDA_VISIBLE_DEVICES="${convnext_gpu}" "${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway raw \
    --config "${config}" --run-root "${run_root}" --backbone convnext_raw --device cuda:0
  CUDA_VISIBLE_DEVICES="${convnext_gpu}" "${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway convnext-p2b \
    --config "${config}" --run-root "${run_root}" --device cuda:0
...
if [[ "${status}" -ne 0 ]]; then
  echo "Four-way experiment failed; inspect logs in ${run_root}." >&2
  exit "${status}"
fi

"${python_bin}" -m PAD_Lite.src.fixed_gallery_fourway summarize \
  --config "${config}" --run-root "${run_root}"
echo "Fixed-Gallery four-way experiment completed: ${run_root}"
